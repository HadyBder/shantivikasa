...
class Store {
 constructor(directory,snapshot){
  ...
  if(version>3)throw new Error('This database was created by a newer app. Please use the newer version.');
  this.db.exec(`CREATE TABLE IF NOT EXISTS products(id TEXT PRIMARY KEY,name TEXT NOT NULL,code TEXT NOT NULL UNIQUE,category TEXT NOT NULL,unit TEXT NOT NULL,icon TEXT NOT NULL,price INTEGER NOT NULL CHECK(price>0),stock INTEGER NOT NULL CHECK(stock>=0),version INTEGER NOT NULL DEFAULT 1,image TEXT NOT NULL DEFAULT '',sourceUrl TEXT NOT NULL DEFAULT '',sourceProductId TEXT NOT NULL DEFAULT '',sourceVariantId TEXT NOT NULL DEFAULT '',stockTracked INTEGER NOT NULL DEFAULT 1,available INTEGER NOT NULL DEFAULT 1,archived INTEGER NOT NULL DEFAULT 0);
   ...
   PRAGMA application_id=${APP_ID}; PRAGMA user_version=3;`);
 ...
 inspectBackup(file){if(fs.statSync(file).size>512*1024*1024)fail('Choose a backup smaller than 512 MB.');const db=new DatabaseSync(file,{readOnly:true});try{if(db.prepare('PRAGMA application_id').get().application_id!==APP_ID||![1,2,3].includes(db.prepare('PRAGMA user_version').get().user_version))fail('This is not a compatible Shanti Vikāsa backup.');if(db.prepare('PRAGMA quick_check').get().quick_check!=='ok')fail('This backup is damaged.');for(const [table,fields] of [['products',productFields],['sales',saleFields],['sale_items',['id',...itemFields]],['metadata',['key','value']]]){const row=db.prepare("SELECT type FROM sqlite_master WHERE name=?").get(table);if(row?.type!=='table')fail('The backup has an invalid structure.');const columns=db.prepare(`PRAGMA table_info(${table})`).all().map(c=>c.name);if(fields.filter(f=>f!=='reference').some(f=>!columns.includes(f)))fail('The backup has an incompatible structure.');}return{products:db.prepare('SELECT COUNT(*) as n FROM products WHERE archived=0').get().n,sales:db.prepare('SELECT COUNT(*) as n FROM sales').get().n};}finally{db.close();}}
 ...
 snapshot(){return{products:this.db.prepare('SELECT * FROM products ORDER BY id').all(),sales:this.db.prepare('SELECT id FROM sales ORDER BY number').all().map(r=>this.sale(r.id))};}
 replaceFromCloud(snapshot){
  ...
  this.transaction(()=>{this.db.exec('DELETE FROM sale_items; DELETE FROM sales; DELETE FROM products;');
