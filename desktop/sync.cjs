...
class SyncEngine{
 ...
 async run({join=false}={}){
  ...
  try{
   if(!this.store.meta('cloud-linked')){
    ...
    const online=await this.request('/api/sync','GET');
   ...
   while(true){
    ...
    const online=await this.request('/api/sync','GET');await this.downloadPhotos(online.snapshot);
 ...
 async resolve(choice){
  ...
  this.running=true;try{
   ...
   const online=await this.request('/api/sync','GET');const remote=online.snapshot.products.find(p=>p.id===event.after.id);
