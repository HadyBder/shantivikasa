...
function setupCloud(store,getWindow,trustedSender,hasDraft){
 ...
 const engine=new SyncEngine(store,async(route,method,body)=>{
  ...
  const response=await ses.fetch(base+route,{method,headers:{'Content-Type':'application/json','Origin':base},credentials:'include',redirect:'error',signal:AbortSignal.timeout(30000),...(body?{body:JSON.stringify(body)}:{})});
