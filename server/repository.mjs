...
export class Repository{
 ...
 async read(){
  const [p,s,r,b,c]=await this.db.batch([this.db.prepare('SELECT payload FROM cloud_products ORDER BY id'),this.db.prepare('SELECT payload FROM cloud_sales'),this.db.prepare('SELECT COALESCE(MAX(revision),0) AS revision FROM cloud_commits'),this.db.prepare('SELECT id FROM cloud_commits WHERE revision=1'),this.db.prepare('SELECT name FROM cloud_categories ORDER BY key')]);
  const revision=r.results[0].revision;
  return{syncProtocol:2,revision,bootstrapId:b.results[0]?.id,initialized:revision>0,snapshot:{categories:categoryList(c.results.map(r=>r.name),p.results.map(r=>JSON.parse(r.payload))),products:p.results.map(r=>JSON.parse(r.payload)),sales:s.results.map(r=>JSON.parse(r.payload)).sort((a,b)=>a.number-b.number)}};
 }
 async seen(id){return !!await this.db.prepare('SELECT id FROM cloud_commits WHERE id=?').bind(id).first();}
 async commit(id,revision,products,saleList,names=[]){
  ...
  for(const name of categoryList(names,products))batch.push(this.db.prepare('INSERT INTO cloud_categories(key,name) VALUES(?,?) ON CONFLICT(key) DO NOTHING').bind(categoryKey(name),name));
 ...
 async bootstrap(body){
  ...
  try{await this.commit(body.id,0,s.products,s.sales,categoryList(s.categories||[],s.products));}catch(e){if(await this.seen(body.id))return{ok:true};if((await this.read()).initialized)fail('Another device imported the register first. No data was overwritten.',409);throw e;}return{ok:true};
 }
 async mutate(id,build){
  for(let attempt=0;attempt<5;attempt++){
   ...
   try{await this.commit(id,current.revision,result.products,result.sale?[result.sale]:[],result.state?.categories||[]);return{ok:true,sale:result.sale};}
