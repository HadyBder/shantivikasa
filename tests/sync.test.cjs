...
async function setup(){
 ...
 const {createClient}=await import('@libsql/client');const client=createClient({url:'file:'+path.join(dir,'online.sqlite')});
 ...
 return{dir,client,store,request,engine,env,handler,close:()=>{store.close();client.close();fs.rmSync(dir,{recursive:true,force:true});}};
}
